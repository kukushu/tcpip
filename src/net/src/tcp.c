 ...
 */
sock_t *tcp_find(ipaddr_t *local_ip, uint16_t local_port, ipaddr_t *remote_ip,
 ...
 * @brief 设置TCP选项
 */
net_err_t tcp_setopt(struct _sock_t *sock, int level, int optname,
...
static void tcp_keepalive_tmo(struct _net_timer_t *timer, void *arg) {
  ...
  if (++tcp->conn.keep_retry <= tcp->conn.keep_cnt) {
    ...
    net_timer_add(&tcp->conn.keep_timer, "keepalive", tcp_keepalive_tmo, tcp,
                  tcp->conn.keep_intvl * 1000, 0);
    dbg_warning(DBG_TCP, "tcp keepalive tmo, retrying: %d",
                tcp->conn.keep_retry);
  } else {
    // 通知应用，TCP已经关闭
 ...
 */
net_err_t tcp_connect(sock_t *sock, const struct x_sockaddr *addr,
                      x_socklen_t len) {
  tcp_t *tcp = (tcp_t *)sock;
 ...
 */
net_err_t tcp_bind(sock_t *sock, const struct x_sockaddr *addr,
                   x_socklen_t len) {
  tcp_t *tcp = (tcp_t *)sock;
...
net_err_t tcp_close(sock_t *sock) {
  ...
  default:
    // 这些状态下已经处于主动关闭或已经完毕
    dbg_error(DBG_TCP, "tcp state error[%s]: send is not allowed",
 ...
 */
net_err_t tcp_send(struct _sock_t *sock, const void *buf, size_t len, int flags,
                   ssize_t *result_len) {
  tcp_t *tcp = (tcp_t *)sock;
  ...
  case TCP_STATE_LAST_ACK:
    ...
    dbg_error(DBG_TCP, "tcp closed[%s]: send is not allowed",
              tcp_state_name(tcp->state));
 ...
 * @brief 执行读请求
 */
net_err_t tcp_recv(struct _sock_t *s, void *buf, size_t len, int flags,
                   ssize_t *result_len) {
  tcp_t *tcp = (tcp_t *)s;
  ...
  default:
    // 监听状态不允许发数据，连接状态未完全建立时也不允许发数据
    // 这种策略虽不符合RFC793，但影响的是只是自己上层的应用程序
    dbg_error(DBG_TCP, "tcp state error[%s]: recv is not allowed",
              tcp_state_name(tcp->state));
    return NET_ERR_STATE;
  }
...
tcp_t *tcp_create_child(tcp_t *parent, tcp_seg_t *seg) {
  ...
  tcp_t *child =
      (tcp_t *)tcp_alloc(0, parent->base.family, parent->base.protocol);
  if (!child) {
